...
ComputeLC2::ComputeLC2(LAMMPS *lmp, int narg, char **arg) :
	Compute(lmp, narg, arg)
{
	if (narg != 5) error->all(FLERR, "Illegal compute LC2 command");

	scalar_flag = 1;

	l = atoi(arg[3]);//TODO check behavior if arg[3] not an integer
	if (l < 1 || l > 8) error->all(FLERR, "Illegal compute LC2 command");
	//TODO see if and when overflows start happening in l

	double cutoff = atof(arg[4]);
	if (cutoff < 0.0) error->all(FLERR, "Illegal compute LC2 command");
	cutsq = cutoff*cutoff;
...
void ComputeLC2::init()
{
	if (force->pair == NULL) 
		error->all(FLERR, "Compute LC2 requires a pair style be defined");
	if (sqrt(cutsq) > force->pair->cutforce) 
		error->all(FLERR, "Compute LC2 cutoff is longer than pairwise cutoff");
	...
	if (2.0*sqrt(cutsq) > force->pair->cutforce + neighbor->skin && comm->me == 0)
		error->warning(FLERR, "Compute LC2 cutoff may be too large to find "
...
void ComputeLC2::init_list(int id, NeighList *ptr)
{
	list = ptr;
}
...
double ComputeLC2::compute_scalar()
{
	...
	if (atom->nlocal > nmax) {
		...
		memory->create(nearest,nmax,MAXNEAR,"LC2:nearest");
		memory->create(nnearest,nmax,"LC2:nnearest");
	}
	...
	if (nerrorall && comm->me == 0) {
		char str[128];
		sprintf(str,"Too many neighbors in LC2 for %d atoms",nerrorall);
...
double ComputeLC2::memory_usage()
{
